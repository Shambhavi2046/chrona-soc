...
class EventNormalizationService:
    ...
    async def ingest_single_event(self, db: AsyncSession, event_in: EventCreate, org_id: Any) -> Any:
        try:
            ...
            result = await event_repository.ingest_single(db, event_in, org_id)
    ...
    async def ingest_bulk_events(self, db: AsyncSession, events_in: List[EventCreate], org_id: Any) -> int:
        try:
            inserted_count = await event_repository.ingest_bulk(db, events_in, org_id)
