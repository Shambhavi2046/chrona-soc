    ...
    <div className="glass-card rounded-xl overflow-hidden flex flex-col h-full relative">
                ...
                <tr key={user.id} className="hover:bg-soc-card-hover transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right relative">
                    ...
                    <div className="absolute right-8 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-soc-card border border-soc-border rounded-lg shadow-lg p-1 z-10 pointer-events-none group-hover:pointer-events-auto">
                      <button onClick={() => openEditModal(user)} disabled={loading} className="p-1.5 text-gray-300 hover:text-white hover:bg-soc-bg rounded transition-colors disabled:opacity-50" title="Edit"><Edit2 className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDisable(user.id)} disabled={loading} className="p-1.5 text-gray-300 hover:text-orange-400 hover:bg-soc-bg rounded transition-colors disabled:opacity-50" title="Disable"><UserX className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDelete(user.id)} disabled={loading} className="p-1.5 text-gray-300 hover:text-red-400 hover:bg-soc-bg rounded transition-colors disabled:opacity-50" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))
        ...
        <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-soc-bg border border-soc-border rounded-xl w-full max-w-md shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-soc-border">
              <h3 className="text-white font-medium">Create New User</h3>
            ...
            <form onSubmit={handleCreateSubmit} className="p-4 space-y-4">
              <div>
                ...
                <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-soc-card border border-soc-border rounded px-3 py-2 text-sm text-white" placeholder="John Doe" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Email</label>
                <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-soc-card border border-soc-border rounded px-3 py-2 text-sm text-white" placeholder="john@example.com" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Password</label>
                <input required minLength={8} type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full bg-soc-card border border-soc-border rounded px-3 py-2 text-sm text-white" placeholder="••••••••" />
              </div>
      ...
      {showEditModal && (
        <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-soc-bg border border-soc-border rounded-xl w-full max-w-md shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-soc-border">
              <h3 className="text-white font-medium">Edit User: {showEditModal.email}</h3>
            ...
            <form onSubmit={handleEditSubmit} className="p-4 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Name</label>
                <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-soc-card border border-soc-border rounded px-3 py-2 text-sm text-white" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Status</label>
                <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="w-full bg-soc-card border border-soc-border rounded px-3 py-2 text-sm text-white">
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setShowEditModal(null)} className="px-4 py-2 text-sm font-medium text-gray-400 hover:text-white">Cancel</button>
