        ...
        {organisations.map((org) => (
          <div key={org.id} className="bg-soc-bg border border-soc-border hover:border-soc-accent/50 rounded-lg p-4 transition-all group relative">
            <div className="flex justify-between items-start mb-3">
              <div>
                <h4 className="text-sm font-bold text-white group-hover:text-soc-accent transition-colors">{org.name}</h4>
            ...
            <div className="space-y-2 mt-4">
              <div className="flex justify-between text-xs text-gray-400">
                <span>Plan</span>
                <span className="text-gray-200 font-medium">{org.plan}</span>
              </div>
              <div className="flex justify-between text-xs text-gray-400">
                <span>Licensed Users</span>
                <span className="text-gray-200 flex items-center"><Users className="w-3 h-3 mr-1" /> {org.users}</span>
              </div>
              <div className="flex justify-between text-xs text-gray-400">
                <span>Created</span>
                <span className="text-gray-200 flex items-center"><Calendar className="w-3 h-3 mr-1" /> {org.created}</span>
              </div>
            </div>
