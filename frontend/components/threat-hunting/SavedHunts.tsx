  ...
  return (
    <div className="glass-card border border-soc-border rounded-xl p-5">
      ...
      {error ? (
        ...
        <div className="text-center p-8 text-soc-text-secondary">No saved hunts yet.</div>
      ) : (
          ...
          {hunts.map((hunt) => (
            <div key={hunt.id} className="bg-soc-bg border border-soc-border hover:border-soc-accent/50 rounded-lg p-4 transition-all group relative overflow-hidden">
                ...
                {editingId === hunt.id ? (
                  <div className="flex items-center gap-2 w-full pr-2">
                    <input 
                      ...
                      className="bg-soc-card border border-soc-accent rounded px-2 py-1 text-sm text-soc-text-primary w-full"
                    />
                  ...
                  <>
                    <h4 className="text-sm font-bold text-soc-text-primary truncate pr-4">{hunt.name}</h4>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 top-2 bg-soc-bg pl-2">
                      <button onClick={() => handleStartEdit(hunt)} className="p-1.5 text-soc-text-secondary hover:text-soc-text-primary hover:bg-soc-card rounded transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
                      <button onClick={() => onDelete && onDelete(hunt.id)} className="p-1.5 text-soc-text-secondary hover:text-red-400 hover:bg-soc-card rounded transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className="px-2 py-0.5 bg-soc-card border border-soc-border rounded text-[10px] font-mono text-soc-text-secondary">
                  {hunt.mitre_mapping || "No Mapping"}
                </span>
                <span className="text-xs text-soc-text-muted flex items-center">
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-soc-border/50">
                <span className="text-xs text-soc-text-muted">By {hunt.author}</span>
