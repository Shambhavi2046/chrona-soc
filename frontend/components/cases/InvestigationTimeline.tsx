...
export default function InvestigationTimeline({ timeline }: InvestigationTimelineProps) {
  const getEventIcon = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'closure': return <Activity className="w-4 h-4 text-soc-text-muted" />;
      ...
      default: return <AlertCircle className="w-4 h-4 text-soc-text-secondary" />;
    }
  };
  ...
  const getEventBadge = (type: string) => {
    ...
    return (
      <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-soc-bg border border-soc-border text-soc-text-secondary">
        {formatted}
      </span>
    );
  };
  ...
  return (
    <div className="glass-card rounded-xl p-6 h-full">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-semibold text-soc-text-primary">Investigation Timeline</h3>
        <button 
      ...
      <div className="relative space-y-6 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-soc-border before:to-transparent">
            ...
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] glass-card p-4 rounded-lg border border-soc-border hover:border-soc-accent/50 transition-colors">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-sm text-soc-text-primary">{event.author}</span>
                  {getEventBadge(event.event_type)}
                </div>
                <time className="text-xs font-mono text-soc-text-muted">
                  <ClientDate date={event.created_at} format="time" />
                </time>
              </div>
              <div className="text-sm text-soc-text-secondary whitespace-pre-wrap bg-soc-card/30 p-3 rounded border border-soc-border/50">
                {event.content}
              </div>
            </div>
          </motion.div>
        ...
        {timeline.length === 0 && (
          <p className="text-soc-text-muted text-center text-sm italic py-4">No events in timeline.</p>
        )}
      </div>
    </div>
  );
