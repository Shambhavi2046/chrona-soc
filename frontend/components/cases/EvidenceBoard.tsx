  ...
  return (
    <div className="glass-card rounded-xl p-6 h-full">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-semibold text-soc-text-primary">Evidence Board</h3>
        <button 
          ...
          <div key={item.id} className="p-4 bg-soc-bg border border-soc-border rounded-lg hover:border-soc-accent/50 transition-colors flex flex-col gap-3">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xs font-medium text-soc-text-muted uppercase tracking-wider block">{item.evidence_type}</span>
                  <p className="text-sm font-mono text-soc-text-primary truncate max-w-[200px]" title={item.value}>{item.value}</p>
                </div>
              </div>
            ...
            {item.description && (
              <p className="text-xs text-soc-text-secondary line-clamp-2 bg-soc-card/50 p-2 rounded border border-soc-border/50">
                {item.description}
              </p>
            )}

            <div className="flex items-center justify-between mt-auto pt-2 border-t border-soc-border text-[10px] text-soc-text-muted">
              <span className="flex items-center gap-1">
                <span className="font-semibold text-soc-text-secondary">Src:</span> {item.source}
              </span>
              <span className="flex items-center gap-1">
                <span className="font-semibold text-soc-text-secondary">By:</span> {item.added_by}
              </span>
            </div>
          </div>
        ))}
        {evidence.length === 0 && (
          <div className="col-span-full p-8 text-center text-soc-text-muted border border-dashed border-soc-border rounded-lg">
