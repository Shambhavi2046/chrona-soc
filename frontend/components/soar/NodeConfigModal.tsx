    ...
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-soc-bg border border-soc-border rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-soc-border bg-soc-card flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-white hover:bg-soc-bg rounded-lg transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="pt-4 border-t border-soc-border">
            ...
            {formData.type === "set_variable" && (
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1">Value (String/JSON)</label>
                  <input
                    type="text"
            ...
            {formData.type === "condition" && (
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1">Expected Value</label>
                  <input
                    type="text"
            ...
            {formData.type === "http_request" && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-400 mb-1">Timeout (s)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1">Body (JSON String)</label>
                  <textarea
                    value={typeof formData.config?.body === 'object' ? JSON.stringify(formData.config.body) : (formData.config?.body || "")}
                    onChange={e => handleConfigChange("body", e.target.value)}
                    className="w-full bg-soc-card border border-soc-border rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-soc-accent min-h-[60px]"
                  />
                </div>
              </div>
            )}
        ...
        <div className="px-6 py-4 border-t border-soc-border bg-soc-card flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 bg-soc-bg border border-soc-border hover:border-gray-500 rounded-lg text-sm font-medium text-gray-300 transition-colors">
            Cancel
          </button>
