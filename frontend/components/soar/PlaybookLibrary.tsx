        ...
        {playbooks.map((playbook) => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <h4 className="text-sm font-bold text-soc-text-primary group-hover:text-soc-accent transition-colors truncate pr-2">{playbook.name}</h4>
            ...
            <p className="text-xs text-soc-text-secondary mt-1 line-clamp-2 h-8">
              {playbook.description}
            </p>

            <div className="mt-3 flex items-center justify-between text-xs text-soc-text-muted">
            ...
            <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-soc-border/50 opacity-0 group-hover:opacity-100 transition-opacity">
              {onDuplicate && (
                <button onClick={(e) => { e.stopPropagation(); onDuplicate(playbook); }} className="p-1.5 text-soc-text-secondary hover:text-soc-text-primary hover:bg-soc-card rounded transition-colors" title="Duplicate">
                  <Copy className="w-3.5 h-3.5" />
                </button>
              )}
              {onDelete && (
                <button onClick={(e) => { e.stopPropagation(); onDelete(playbook.id); }} className="p-1.5 text-soc-text-secondary hover:text-red-400 hover:bg-red-500/10 rounded transition-colors" title="Delete">
              ...
              {playbook.status === "Active" ? (
                onDeactivate && (
                  <button onClick={(e) => { e.stopPropagation(); onDeactivate(playbook.id); }} className="flex items-center gap-1 px-3 py-1.5 bg-soc-card hover:bg-soc-bg border border-soc-border hover:border-gray-500 rounded text-xs font-medium text-soc-text-secondary transition-colors">
