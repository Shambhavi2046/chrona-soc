    ...
    <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} mb-6`}>
      ...
      <div className={`flex flex-col max-w-[85%] md:max-w-[75%]`}>
        <div className={`flex items-center gap-2 mb-1 ${isUser ? 'justify-end' : 'justify-start'}`}>
          <span className="text-xs font-bold text-soc-text-secondary">
            {isUser ? 'You' : 'Chrona Copilot'}
          </span>
          <span className="text-[10px] text-soc-text-muted font-mono">
        ...
        <div className={`relative px-5 py-4 rounded-2xl group ${
          ...
          {/* Markdown Content */}
          <div className="prose prose-invert prose-sm max-w-none prose-headings:text-soc-text-primary prose-a:text-soc-accent hover:prose-a:text-blue-400 prose-code:text-soc-warning prose-code:bg-soc-bg prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-soc-bg prose-pre:border prose-pre:border-soc-border">
            ...
            <div className="mt-4 pt-3 border-t border-soc-border/50 flex flex-wrap gap-2">
              {quickActions.map((action, idx) => (
                <Link 
                  key={idx} 
                  href={action.url}
                  className="inline-flex items-center px-3 py-1.5 bg-soc-bg/50 hover:bg-soc-accent/20 border border-soc-border hover:border-soc-accent rounded-full text-xs font-semibold text-soc-text-secondary hover:text-white transition-all group"
                >
          ...
          {!isUser && (
            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                ...
                className="p-1.5 text-soc-text-muted hover:text-soc-text-primary hover:bg-soc-bg rounded transition-colors"
              ...
              {isLatest && onRegenerate && (
                <button 
                  ...
                  className="p-1.5 text-soc-text-muted hover:text-soc-accent hover:bg-soc-bg rounded transition-colors"
      ...
      {isUser && (
        <div className="flex-shrink-0 ml-4">
          <div className="w-8 h-8 rounded-full bg-soc-card border border-soc-border flex items-center justify-center text-soc-text-secondary">
